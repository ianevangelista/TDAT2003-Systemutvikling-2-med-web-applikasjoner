...
app.post("/token", (req, res) => {
  jwt.verify(req.headers["x-access-token"], publicKey, (err, decoded) => {
    if (err) {
      ...
      var user = decoded.brukernavn;
      console.log("Token ok: " + decoded.brukernavn);
      let token = jwt.sign({ brukernavn: user }, privateKey, {
        expiresIn: 10
      });
      res.json({ jwt: token });
    }
...
app.use("/api", (req, res, next) => {
  console.log("Kjører FØR alle endepunkt mountet under /api");
