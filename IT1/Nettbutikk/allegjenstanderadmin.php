...
if ( isset( $_GET["sell"]) ){
    $sql = sprintf("UPDATE gjenstand SET status = 1 WHERE gjenstandid = %s", $_GET["sell"]);
...
AS 'utlopsdato', bruker.fornavn, bruker.etternavn, gjenstand.brukerid, gjenstand.minpris, MAX(verdi) AS hoyeste 

FROM gjenstand, kategori, bruker, bud 
WHERE gjenstand.kategoriid = kategori.kategoriid AND gjenstand.brukerid = bruker.brukerid AND status = '0'
AND CURRENT_TIMESTAMP < utlopsdato GROUP BY gjenstandid");
$datasett = $tilkobling->query($sql);
?>
    ...
    <div id="wrapper">
        ...
        <main>
            ...
            <tr>
                ...
                <td><?php echo $rad["hoyeste"]; ?></td>
                   <td> <a href="?sell=<?php echo $rad["gjenstandid"]?>">Selg</a>
                </td>
            </tr>
            <?php } ?>
